...
class SmartTableParser:
    ...
    def parse_asset_allocation_table(self, table: Tag) -> List[Dict[str, Any]]:
        ...
        for row in rows[data_start_idx:]:
            ...
            # 提取资产类型
            if "asset_type" in header_mapping and header_mapping["asset_type"] < len(
                cells
            ):
                asset_type = self.extract_cell_value(
                    cells[header_mapping["asset_type"]]
                )
                if asset_type:
            ...
            # 提取市值
            if "market_value" in header_mapping and header_mapping[
                "market_value"
            ] < len(cells):
            ...
            # 提取占比
            if "percentage" in header_mapping and header_mapping["percentage"] < len(
                cells
            ):
    ...
    def parse_top_holdings_table(self, table: Tag) -> List[Dict[str, Any]]:
        ...
        for row in rows[data_start_idx:]:
            ...
            # 提取证券名称
            if "security_name" in header_mapping and header_mapping[
                "security_name"
            ] < len(cells):
                security_name = self.extract_cell_value(
                    cells[header_mapping["security_name"]]
                )
                if security_name:
                    holding["security_name"] = security_name

            # 提取证券代码
            if "security_code" in header_mapping and header_mapping[
                "security_code"
            ] < len(cells):
                security_code = self.extract_cell_value(
                    cells[header_mapping["security_code"]]
                )
                if security_code:
            ...
            if "shares" in header_mapping and header_mapping["shares"] < len(cells):
                shares = self.extract_cell_value(
                    cells[header_mapping["shares"]], "number"
                )
                if shares:
    ...
    def parse_industry_allocation_table(self, table: Tag) -> List[Dict[str, Any]]:
        ...
        for row in rows[data_start_idx:]:
            ...
            # 提取行业名称
            if "industry_name" in header_mapping and header_mapping[
                "industry_name"
            ] < len(cells):
                industry_name = self.extract_cell_value(
                    cells[header_mapping["industry_name"]]
                )
                if industry_name:
                    allocation["industry_name"] = industry_name

            # 提取行业代码
            if "industry_code" in header_mapping and header_mapping[
                "industry_code"
            ] < len(cells):
                industry_code = self.extract_cell_value(
                    cells[header_mapping["industry_code"]]
                )
                if industry_code:
    ...
    def _find_data_start_index(self, rows: List[Tag]) -> int:
        ...
        for i, row in enumerate(rows):
            ...
            if any(
                ...
                for keyword in ["序号", "名称", "代码", "数量", "金额", "比例", "类别", "行业"]
            ):
                continue
...
class XBRLParser:
    ...
    def _read_file_with_encoding(self, file_path: Path) -> Optional[str]:
        ...
        try:
            ...
            self.logger.warning(
                "xbrl_parser.file_read_error", file_path=str(file_path), error=str(e)
            )
            return None

    def _parse_basic_info(
        self, soup: BeautifulSoup, fund_data: ParsedFundData, bound_logger
    ):
        """解析基本信息"""
        ...
        # 提取基金管理人
        fund_manager = self._extract_by_patterns(
            text_content, self.patterns["fund_manager"]
        )
        if fund_manager:
            fund_data.fund_manager = fund_manager.strip()
            bound_logger.info(
                "xbrl_parser.basic_info.fund_manager", manager=fund_manager[:50]
            )
    ...
    def _parse_nav_info(
        ...
        if nav:
            try:
                ...
                bound_logger.info(
                    "xbrl_parser.nav_info.nav", value=fund_data.net_asset_value
                )
            except (InvalidOperation, ValueError) as e:
                bound_logger.warning(
                    "xbrl_parser.nav_info.nav_parse_error", value=nav, error=str(e)
                )

        # 提取总净资产
        total_assets = self._extract_by_patterns(
            text_content, self.patterns["total_net_assets"]
        )
        if total_assets:
            try:
                fund_data.total_net_assets = Decimal(total_assets.replace(",", ""))
                bound_logger.info(
                    "xbrl_parser.nav_info.total_assets",
                    value=fund_data.total_net_assets,
                )
            except (InvalidOperation, ValueError) as e:
                bound_logger.warning(
                    "xbrl_parser.nav_info.assets_parse_error",
                    value=total_assets,
                    error=str(e),
                )
    ...
    def _parse_investment_data(
        ...
        for table in tables:
            ...
            if table_type == "asset_allocation":
                ...
                bound_logger.info(
                    "xbrl_parser.asset_allocation.parsed", count=len(allocations)
                )
                ...
                bound_logger.info(
                    "xbrl_parser.top_holdings.parsed", count=len(holdings)
                )
                ...
                bound_logger.info(
                    "xbrl_parser.industry_allocation.parsed", count=len(allocations)
                )
