...
class FundReportService:
        ...
        try:
            ...
            result = {
                ...
                "criteria": {
                    ...
                    "report_type_name": ReportType.get_description(
                        criteria.report_type
                    ),
                    "fund_type": criteria.fund_type.value
                    if criteria.fund_type
                    else None,
                    "fund_type_name": FundType.get_description(criteria.fund_type)
                    if criteria.fund_type
                    else None,
            ...
            bound_logger.info(
                "fund_report_service.search_reports.success", total_found=len(reports)
            )
    ...
    async def search_all_pages(
        self, criteria: FundSearchCriteria, max_pages: Optional[int] = None
    ) -> Dict:
        """
        获取所有页面的报告
        ...
        """
        bound_logger = logger.bind(
            criteria=criteria.get_description(), max_pages=max_pages
        )
        ...
        try:
            while True:
                ...
                if not reports:
                    bound_logger.info(
                        "fund_report_service.search_all_pages.no_more_data", page=page
                    )
                    break
    ...
    def download_report(self, report: Dict, save_dir: Path) -> Dict:
        ...
        try:
            ...
            bound_logger.error(
                "fund_report_service.download_report.missing_key", error=error_msg
            )
            return {
                "success": False,
                "error": error_msg,
