...
from sqlalchemy import Column, Integer, String, DateTime, Date, Text, Numeric, ForeignKey, Index
...
class FundReport(Base):
    ...
    # 原始文件信息
    upload_info_id = Column(String(50), nullable=False, unique=True, index=True)  # 原始上传ID
    ...
    # 关联关系
    asset_allocations = relationship("AssetAllocation", back_populates="fund_report", cascade="all, delete-orphan")
    top_holdings = relationship("TopHolding", back_populates="fund_report", cascade="all, delete-orphan")
    industry_allocations = relationship("IndustryAllocation", back_populates="fund_report", cascade="all, delete-orphan")
...
class AssetAllocation(Base):
    ...
    # 外键关联
    fund_report_id = Column(Integer, ForeignKey('fund_reports.id'), nullable=False, index=True)
        ...
        "tables": [
            {
                ...
                "key_fields": ["fund_code", "report_period_end", "upload_info_id", "total_net_assets", "net_asset_value"]
            },
            {
            ...
            },
            {
                ...
                "key_fields": ["security_code", "security_name", "market_value", "percentage", "rank"]
            },
            {
