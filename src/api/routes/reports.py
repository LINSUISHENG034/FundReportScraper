...
async def search_reports(
    ...
    fund_type: Optional[str] = Query(
        None, description="基金类型代码（可选）", example="6020-6050"
    ),
    fund_company_short_name: Optional[str] = Query(
        None, description="基金管理人简称（可选）", example="工银瑞信"
    ),
    fund_code: Optional[str] = Query(None, description="基金代码（可选）", example="164906"),
    fund_short_name: Optional[str] = Query(
        None, description="基金简称（可选）", example="工银全球"
    ),
    start_upload_date: Optional[date] = Query(
        None, description="开始上传日期（可选）", example="2024-01-01"
    ),
    end_upload_date: Optional[date] = Query(
        None, description="结束上传日期（可选）", example="2024-12-31"
    ),
    ...
    bound_logger = logger.bind(
        year=year, report_type=report_type, page=page, page_size=page_size
    )
    ...
    try:
        ...
        try:
            ...
            valid_types = [
                f"{rt.value} ({ReportType.get_description(rt)})" for rt in ReportType
            ]
        ...
        if fund_type:
            try:
                ...
                valid_types = [
                    f"{ft.value} ({FundType.get_description(ft)})" for ft in FundType
                ]
        ...
        if (
            start_upload_date
            and end_upload_date
            and start_upload_date > end_upload_date
        ):
        ...
        search_criteria_info = {
            ...
            "fund_type": fund_type,
            "fund_type_name": FundType.get_description(fund_type_enum)
            if fund_type_enum
            else None,
            "fund_company_short_name": fund_company_short_name,
            "fund_code": fund_code,
            "fund_short_name": fund_short_name,
            "start_upload_date": start_upload_date.isoformat()
        ...
        bound_logger.error(
            "api.reports.search.error", error=str(e), error_type=type(e).__name__
        )
...
async def get_fund_types():
    ...
    return {
        "success": True,
        "data": [
            {"code": ft.value, "name": FundType.get_description(ft), "value": ft.value}
            for ft in FundType
        ],
    }
