...
@cli.command()
@click.option('--report-type', type=click.Choice(['ANNUAL', 'SEMI_ANNUAL', 'QUARTERLY', 'FUND_PROFILE']), required=True, help='报告类型 (QUARTERLY, ANNUAL, etc.)')
@click.option('--fund-type', type=click.Choice([e.value for e in FundType]), help='基金类型')
@click.option('--year', type=int, default=datetime.now().year, help='报告年份')
@click.option('--quarter', type=click.IntRange(1, 4), help='报告季度 (当 report-type 为 QUARTERLY 时必须提供)')
...
def download(report_type: str, fund_type: str, year: int, quarter: Optional[int], fund_code: Optional[str], fund_name: Optional[str], save_dir: str, limit: int):
    """根据条件搜索并下载基金报告"""
    click.echo("--- 开始下载任务 ---")
