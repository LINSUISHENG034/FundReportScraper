...
async def main():
    ...
    criteria = FundSearchCriteria(
        year=2024, report_type=ReportType.ANNUAL, fund_code="015975"
    )
        ...
        if search_result and search_result.get("success"):
            ...
            if reports_to_download:
                ...
                for report in reports_to_download:
                    logging.info(
                        f"  - 待下载报告: fund_code={report.get('fund_code')}, upload_info_id={report.get('upload_info_id')}"
                    )
            else:
    ...
    try:
        logging.info(
            f"探针[3]: 即将调用 fund_report_service.download_report for {report_to_download.get('upload_info_id')}..."
        )
        download_result = await fund_report_service.download_report(
            report_to_download, save_dir
        )
