...
class FundSearchCriteria:
    ...
    def to_ao_data_params(self) -> Dict[str, Any]:
        ...
        report_year = "" if self.report_type == ReportType.FUND_PROFILE else str(self.year)
        ...
        return {
            ...
            "startUploadDate": self.start_upload_date.strftime("%Y-%m-%d") if self.start_upload_date else "",
            "endUploadDate": self.end_upload_date.strftime("%Y-%m-%d") if self.end_upload_date else ""
        }
    ...
    def get_description(self) -> str:
        ...
        desc_parts = [
            f"{self.year}年",
            ReportType.get_description(self.report_type)
        ]
...
COMMON_FUND_COMPANIES = [
    "工银瑞信", "华夏", "嘉实", "南方", "博时", "广发", "大成", "富国", "国泰", "华安",
    "建信", "交银施罗德", "农银汇理", "中银", "招商", "中欧", "易方达", "鹏华", "汇添富", "银华"
]
...
def generate_test_cases() -> list:
    ...
    for report_type in ReportType:
        test_cases.append(FundSearchCriteria(
            year=2024,
