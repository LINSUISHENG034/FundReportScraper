...
class AnnualReportBatchProcessor:
    ...
    async def get_all_annual_reports(
        ...
        while True:
            if max_pages and page > max_pages:
                logger.info(
                    "annual_processor.max_pages_reached",
                    page=page,
                    max_pages=max_pages
                )
                break
    ...
    async def batch_download_and_parse(
        ...
        # 并发处理所有报告
        tasks = [
            process_single_report(report, i) 
        ...
        for result in completed_results:
            if isinstance(result, Exception):
                ...
                if result.get('download_success'):
                    ...
                    results['statistics']['total_file_size'] += result.get('file_size', 0)
    ...
    async def _process_single_report(
        ...
        try:
            ...
            file_path = self._save_report_file(
                content, fund_code, "ANNUAL", save_dir
            )
            ...
            if parse_xbrl:
                ...
                try:
                    ...
                    result.update({
                        'parsing_success': parsing_result.get('parsing_success', False),
            ...
            logger.error(
                "annual_processor.report_failed",
                fund_code=fund_code,
                error=str(e)
            )
    ...
    def _save_batch_results(self, results: Dict, save_dir: Path):
        ...
        logger.info(
            "annual_processor.results_saved",
...
async def full_batch_download():
    ...
    try:
        ...
        for company, count in sorted(fund_companies.items(), key=lambda x: x[1], reverse=True)[:5]:
            print(f"     {company}: {count} 只基金")

        # 询问用户是否继续
        print(f"\n⚠️  即将下载 {len(all_reports)} 个基金的年度报告")
        print(f"   预估文件大小: {len(all_reports) * 100} KB - {len(all_reports) * 500} KB")
        print(f"   预估下载时间: {len(all_reports) * 2 // 60} - {len(all_reports) * 5 // 60} 分钟")
        ...
        print(f"   解析成功: {results['successful_parsing']}")
        print(f"   解析失败: {results['failed_parsing']}")
        print(f"   成功率: {(results['successful_downloads'] / results['total_reports'] * 100):.1f}%")
        print(f"   总文件大小: {results['statistics']['total_file_size'] / 1024 / 1024:.1f} MB")
