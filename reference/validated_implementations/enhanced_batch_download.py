...
class EnhancedFundReportProcessor:
    ...
    async def batch_download(
        self, reports: List[Dict], save_dir: Path, max_concurrent: int = 3
    ) -> Dict:
        """批量下载报告"""
...
async def test_enhanced_search():
    ...
    async with EnhancedFundReportProcessor() as processor:
        ...
        criteria1 = FundSearchCriteria(
            year=2024, report_type=ReportType.ANNUAL, page_size=10
        )
        ...
        criteria4 = FundSearchCriteria(
            year=2024, report_type=ReportType.ANNUAL, fund_code="001648", page_size=10
        )
...
async def test_batch_download():
    ...
    async with EnhancedFundReportProcessor() as processor:
        ...
        if reports:
            save_dir = Path("data/downloads/enhanced_batch")
