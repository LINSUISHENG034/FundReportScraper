...
class TestXBRLParser:
        ...
        return {
            ...
            "complex_quarterly_report": {
                ...
                "data_quality_requirements": {
                    ...
                    "industry_allocations": {"required_keys": ["industry_name"]},
                },
                "required_fields": ["fund_code"],
                "optional_fields": [
                    "fund_name",
    ...
    def _validate_data_counts(
        self, result: ParsedFundData, expected_data: Dict[str, Any]
    ):
        """验证数据结构的数量范围"""
        count_validations = [
        ...
        for field_name, data_list in count_validations:
            ...
            if count_key in expected_data:
                ...
                assert count_range["min"] <= actual_count <= count_range["max"], (
                    f"{field_name} 数量 {actual_count} 不在预期范围 "
                    f"[{count_range['min']}, {count_range['max']}] 内"
                )

    def _validate_data_quality(
        self, result: ParsedFundData, quality_requirements: Dict[str, Any]
    ):
        """验证数据质量要求"""
        # 验证资产配置数据质量
        ...
        if "top_holdings" in quality_requirements:
            ...
            if req.get("rank_sequence", False):
                ranks = [
                    item.get("rank")
                    for item in result.top_holdings
    ...
    def _validate_list_data_quality(
        self, data_list: List[Dict], requirements: Dict, data_type: str
    ):
        """验证列表数据的质量"""
        if not data_list:
            return
    ...
    @pytest.mark.parametrize(
        "file_name,expected_fund_code,expected_fund_name",
        [
            (
                "013060_ANNUAL_1752537343.xbrl",
                "013060",
                ("工银瑞信养老目标日期2060五年持有期混合型发起式基金中基金（FOF）"),
            ),
            ("970196_2023Q3_1752541804.xbrl", "970196", "诚通天天利货币"),
            (
                "003016_SEMI_ANNUAL_1752539909.xbrl",
                "003016",
                "中金中证500指数增强型发起",
            ),
    ...
    def test_specific_file_parsing_accuracy(
        self, parser, fixtures_dir, file_name, expected_fund_code, expected_fund_name
    ):
        """测试特定文件的解析精度"""
        ...
        # 验证基金代码
        assert (
            result.fund_code == expected_fund_code
        ), f"基金代码不匹配: 期望 {expected_fund_code}, 实际 {result.fund_code}"

        # 验证基金名称（如果存在）
        if result.fund_name:
            assert (
                expected_fund_name in result.fund_name
    ...
    def test_parsed_fund_data_initialization(self):
        ...
        data_with_params = ParsedFundData(
            fund_code="000001", fund_name="测试基金", report_year=2024
        )
